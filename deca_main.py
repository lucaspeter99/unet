...
data_dir = './deca_data/Task10_Colon/Task10_Colon/'
results_dir = './results'

dataset_classes = {
    0: deca_datasets.Colon
}
...
train_dataset = DatasetClass(data_dir, training = True, rotate=rotate, deform=deform, alpha=alpha, sigma=sigma)
test_dataset = DatasetClass(data_dir, training = False, rotate=rotate, deform=deform, alpha=alpha, sigma=sigma)
...
val_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=workers)

date = datetime.now().strftime('%Y-%m-%d')
name = f'{tag}_{date}_{num_epochs}_bs{batch_size}_lr{lr}{"_DecayTo_" if lr_decay else ""}{lr_min if lr_decay else ""}'
name += f'_{gen.__class__.__name__}_{opt_gen.__class__.__name__}_{criterion.__class__.__name__}_{train_dataset.__class__.__name__}'
...
def startup_logs():
    ...
    logger.log('\n== Data ==')
    logger.log(f'Dataset: {test_dataset.__class__.__name__}')
    ...
    logger.log(f'Validation Dataset Size: {len(test_dataset)}')
    logger.log('\n== Routine ==')
...
def train_unet():
    ...
    compute_metrics(data_obj=train_dataset, key='training', generator=gen, metrics=metrics, plot_dir=plot_dir)
    compute_metrics(data_obj=test_dataset, key='validation', generator=gen, metrics=metrics, plot_dir=plot_dir)
