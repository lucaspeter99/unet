...
def create_and_train_unet(tag, dc_key, lr, num_epochs, mse, batchnorm, leaky, max_pool, start_data, use_medical_data, split_med_channels,
                          only_split_tici, split_modalities, windowing, use_gpu, batch_size, seed, epoch_sample_interval):
    ...
    if 'll337mdee' in os.getcwd():
        data_dir = "/work/users/ll337mdee/train_data/"
        val_data_dir = "/work/users/ll337mdee/val_data/"
        results_dir = './lesions/results'
    else:
        data_dir = './train_data'
        val_data_dir = './val_data'
        results_dir = './results'
    ...
    if use_medical_data: #can only split be reca if med data is used
        ...
        x_train, x_test, y_train, y_test = model_selection.train_test_split(list(range(len(data))), reca_statuses, random_state=seed, shuffle= True, test_size=(1/6),stratify=reca_statuses)
        ...
        x_train, x_test = model_selection.train_test_split(list(range(len(data))),random_state=seed, shuffle=True,test_size=(1/6))
        dataset = Subset(data, x_train)  # training set
        val_dataset = Subset(data, x_test)  # test set
    ...
    if num_gpu > 1: # if multiple gpus are available
        print("Parallelizing model...")
        #gen = torch.nn.DataParallel(gen)
        gen = torch.nn.parallel.DistributedDataParallel(gen)
    ...
    name += f'_{gen_name}_{opt_gen.__class__.__name__}_{criterion.__class__.__name__}_{data.__class__.__name__}{"_windowed" if windowing else ""}'
    ...
    def startup_logs():
        ...
        if use_gpu:
            ...
            logger.log(f'Device count:{num_gpu}')
    ...
    def compute_metrics(data_obj, key, generator, metrics, plot_dir, thresholds, ml_per_voxel, sample_dir ,best_threshold = None ):
    ...
    def run_stats(dataset, val_dataset, gen, sample_dir, plot_dir, ml_per_voxel):
        # save the model
        torch.save(gen, os.path.join(sample_dir, 'gen.trc'))
        ...
        compute_metrics(data_obj=dataset, key='training', generator=gen, metrics=metrics, plot_dir=plot_dir,
                        ml_per_voxel=ml_per_voxel,
                        thresholds=possible_thresholds.copy(), sample_dir =sample_dir)  # get metrics, plot for those thresholds on training set
        # get best threshold
        ...
        compute_metrics(data_obj=val_dataset, key='validation', generator=gen, metrics=metrics, plot_dir=plot_dir,
                        ...
                        best_threshold=float(best_threshold_on_training_set), sample_dir = sample_dir)
        with open(os.path.join(sample_dir, 'metrics.json'), 'w') as file:
    ...
    def train_unet():
        ...
        for epoch in range(num_epochs):
            ...
            if ((epoch + 1) % epoch_sample_interval) == 0 or ((epoch+1) == num_epochs):
                sample_dir = os.path.join(results_dir, "epoch" + str(epoch+1))
...
if __name__ == '__main__':
    # runtime variables
    tag = str(sys.argv[1]) if len(sys.argv) > 1 else 'SEED555'  # project name tag
    ...
    num_epochs = int(sys.argv[4]) if len(sys.argv) > 4 else 50
    ...
    batch_size = int(sys.argv[16]) if len(sys.argv) > 16 else 5

    seed = int(sys.argv[17]) if len(sys.argv) > 17 else 555
    epoch_sample_interval = int(sys.argv[18]) if len(sys.argv) > 18 else 50
    create_and_train_unet(tag, dc_key, lr, num_epochs, mse, batchnorm, leaky, max_pool, start_data, use_medical_data,
                          split_med_channels,
                          only_split_tici, split_modalities, windowing, use_gpu, batch_size, seed, epoch_sample_interval)
